...
func TestNewClient(t *testing.T) {
	...
	// Set the credentials file for all the tests to avoid causing failures when there is no
	// credentials file on the system where the tests are running.
	...
	t.Run("No project when using VertexAI", func(t *testing.T) {
	...
	t.Run("No location when using VertexAI", func(t *testing.T) {
		unsetEnvVars(t)
	...
	t.Run("Check initialization of Models", func(t *testing.T) {
		...
		opts := []cmp.Option{
			...
			cmp.Comparer(func(x, y *google.Credentials) bool {
				// Validating against nil is enough.
				return x != nil && y != nil
			}),
		}
